...
public class App implements Runnable {
    ...
    @Option(order = 4, names = {"-o", "--output"}, defaultValue = "./output/", paramLabel = "OUT",
                ...
                if (e < 0 || e > 4) {
                    ...
                    for (XTrace trace : log) {
                        ...
                        switch (Encoding.get(e)) {
                            case GENERAL:
                                FileUtils.copyFile(new File("lib/src/main/resources/domain-general.pddl"),
                                        new File(output_location, "domain-general.pddl"));
                                enc = new GeneralEncoding("general", trace_aut, tempConstraint,
                                        true);
                                break;
                            case GENERAL_CONJ:
                                FileUtils.copyFile(new File("lib/src/main/resources/domain-general-conj.pddl"),
                                        new File(output_location, "domain-general-conj.pddl"));
                                enc = new GeneralEncodingConjunctiveGoal("general-conj", trace_aut,
                                        tempConstraint, true);
                                break;
                            case GENERAL_SHARE:
                                FileUtils.copyFile(new File("lib/src/main/resources/domain-general-share.pddl"),
                                        new File(output_location, "domain-general-share.pddl"));
                                enc = new GeneralEncodingShareStates("general-share", trace_aut,
                                        tempConstraint, true);
                                break;
                            case GENERAL_CONJ_SHARE:
                                FileUtils.copyFile(new File("lib/src/main/resources/domain-general-conj-share.pddl"),
                                        new File(output_location, "domain-general-conj-share.pddl"));
