...
public class GeneralEncoding extends AbstractEncoding {
    ...
    public Domain generateDomain() {
        ...
        d.addRequirement(RequireKey.DISJUNCTIVE_PRECONDITIONS);
    ...
    private void _addInit(Problem p) {
        ...
        _addTraceAutomaton(p);
        _addConstraints(p);
    }

    private void _addTraceAutomaton(Problem p) {
    ...
    private void _addConstraints(Problem p) {
        int j = 0;
        for (Automaton<String> a : this.constraint_automata) {
    ...
    private void _addGoal(Problem p) {
        ...
        TypedSymbol s = new TypedSymbol(new Symbol(Symbol.Kind.VARIABLE, "?s"));
        s.addType(new Symbol(Symbol.Kind.TYPE, "state"));
        forall_goal.setVariables(Collections.singletonList(s));
...
//            ));
//
//            TraceAutomaton<String> t = new TraceAutomaton<>(al, ss, ts, new HashMap<>());
//
//            String constraint = "G(a -> (X b))";
...
////            System.out.println(a.computeAutomaton(al));
...
//
//            GeneralEncoding ae = new GeneralEncoding("gen0", t, c, true);
